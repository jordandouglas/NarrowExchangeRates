...
public class NEROperator_dAB_dAE_dBC extends MetaNEROperator {
	...
	protected double proposalRates(double rWindowSize, double ta, double tb, double tc, double td, double te, 
		...
		this.rbp = -(te*(r_delta + rd) + rb*tb - rb*td - rd*td + rd*te - (r_delta + rd)*(r_epsilon + td))/(r_epsilon - tb + td);
		this.rcp = (te*(r_delta + rd) - rc*tc + rc*te - 2*rd*td + 2*rd*te - (r_delta + rd)*(r_epsilon + td))/(r_epsilon - tc + td);
		this.rdp = r_delta + rd;
